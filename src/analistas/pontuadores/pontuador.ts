...
export function scoreArquetipo(
  ...
  if (!explicacaoSimilaridade) {
    ...
    if (missingRequired.length > 0) {
      partes.push(
        `Diretórios obrigatórios ausentes/faltantes: ${missingRequired.join(', ')}.`,
      );
    }
    if (forbiddenPresent.length > 0) {
      partes.push(
        `Diretórios não permitidos/proibidos presentes: ${forbiddenPresent.join(', ')}.`,
      );
    }
    if (partes.length > 0) {
...
export function scoreArquetipoAvancado(
  ...
  const maxPossible =
    ...
    (def.dependencyHints?.length || 0) * PESO_DEPENDENCIA +
    (def.filePresencePatterns?.length || 0) * PESO_PATTERN +
    100; // Adiciona margem para análise avançada
