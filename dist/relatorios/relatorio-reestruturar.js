...
export async function gerarRelatorioReestruturarMarkdown(caminho, movimentos, opcoes) {
    ...
    linhas.push('');
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.metadados.data}:** ${dataISO}  `);
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.metadados.execucao}:** ${simulado ? RelatorioMessages.reestruturar.secoes.metadados.simulacao : RelatorioMessages.reestruturar.secoes.metadados.real}  `);
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.metadados.origemPlano}:** ${origem}  `);
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.metadados.preset}:** ${preset}  `);
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.movimentos.total}:** ${total}  `);
    linhas.push(`**${RelatorioMessages.reestruturar.secoes.conflitos.total}:** ${conflitos}  `);
    linhas.push('');
    linhas.push('---');
    linhas.push('');
