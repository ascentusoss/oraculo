...
function assertInsideRoot(caminho) {
    ...
    if (!resolved.startsWith(path.resolve(ROOT))) {
        throw new Error(ExcecoesMessages.persistenciaNegadaForaRaizProjeto(caminho));
    }
}
...
async function salvarEstadoImpl(caminho, dados) {
    ...
    const tempFilePath = path.join(dir, `.tmp-bin-${Date.now()}-${Math.random().toString(16).slice(2)}.oraculo`);
...
export async function salvarEstadoAtomico(caminho, dados) {
    ...
    const tempFilePath = path.join(dir, `.tmp-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
...
export async function salvarBinarioAtomico(caminho, dados) {
    ...
    const dir = path.dirname(caminho);
    await mkdirSafe(dir, { recursive: true, mode: 0o700 });
    const temporaryValue = path.join(dir, `.tmp-${Date.now()}-${Math.random().toString(16).slice(2)}.bin`);
    const p = fs;
    ...
    if (typeof cbWrite === 'function') {
        await new Promise((resolve, reject) => {
            cbWrite(temporaryValue, dados, (err) => (err ? reject(err) : resolve()));
        });
        await renameSafe(temporaryValue, caminho);
        return;
    }
