...
export function comandoMetricas() {
    return new Command('metricas')
        ...
        .option('-l, --limite <n>', 'Quantidade de registros mais recentes (default 10)', (v) => Number(v), 10)
        ...
        try {
            ...
            for (const h of ultimos) {
                ...
                log.info(CliComandoMetricasMessages.linhaExecucao(timestampISO, h.totalArquivos ?? 0, formatarDuracao(h.tempoAnaliseMs ?? 0), formatarDuracao(h.tempoParsingMs ?? 0), h.cacheAstHits ?? 0, h.cacheAstMiss ?? 0));
            }
            if (opts.analistas && agg) {
                log.info(CliComandoMetricasMessages.linhaEmBranco);
                log.info(CliComandoMetricasMessages.tituloTopAnalistas(ICONES_DIAGNOSTICO.info));
                for (const a of agg.topAnalistas) {
                    log.info(CliComandoMetricasMessages.linhaTopAnalista(a.nome, formatMs(a.totalMs), formatMs(a.mediaMs), a.execucoes, a.ocorrencias));
                }
            }
            if (agg) {
                log.info(CliComandoMetricasMessages.medias(formatMs(agg.mediaAnaliseMs), formatMs(agg.mediaParsingMs)));
            }
        }
        catch (err) {
