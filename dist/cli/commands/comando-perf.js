...
async function gerarBaseline(destDir, metricas) {
    ...
    const snapshot = { ...base, hashConteudo };
...
export function comandoPerf() {
    ...
    return new Command('perf')
        ...
        .description('Gera uma nova baseline. Usa métricas globais da última execução se disponíveis.')
        ...
        try {
            ...
            const metricas = globalThis.__ULTIMAS_METRICAS_ORACULO__;
        ...
        if (snaps.length < 2) {
            ...
            if (parent.json)
                console.log(JSON.stringify({ erro: msg }));
            else
                log.aviso(msg);
            return;
        }
        ...
        if (parent.json) {
            ...
            log.info(CliComandoPerfMessages.tituloComparacaoSnapshotsComIcone(ICONES_DIAGNOSTICO.info));
            diffs.forEach((d) => {
