...
import { DICAS, formatarTipoInseguro, gerarResumoCategoria, ICONES_FIX_TYPES as ICONES, log, MENSAGENS_AUTOFIX, MENSAGENS_CLI_FIX_TYPES, MENSAGENS_ERRO_FIX_TYPES as MENSAGENS_ERRO, MENSAGENS_INICIO_FIX_TYPES as MENSAGENS_INICIO, MENSAGENS_PROGRESSO_FIX_TYPES as MENSAGENS_PROGRESSO, MENSAGENS_RESUMO, MENSAGENS_SUCESSO_FIX_TYPES as MENSAGENS_SUCESSO, TEMPLATE_RESUMO_FINAL, TEXTOS_CATEGORIZACAO_FIX_TYPES, } from '../../core/messages/index.js';
...
export function criarComandoFixTypes() {
    ...
    cmd
        ...
        .option('--dry-run', 'Mostra o que seria corrigido sem aplicar mudanças', false)
        .option('--target <path>', 'Diretório ou arquivo específico para analisar', 'src')
        .option('--confidence <number>', 'Nível mínimo de confiança para aplicar correções (0-100)', '85')
        .option('--verbose', 'Mostra detalhes de cada correção', false)
        .option('--interactive', 'Modo interativo: confirma cada correção', false)
        .option('--export', 'Exporta relatórios JSON e Markdown para pasta relatorios/', false)
...
async function executarFixTypes(options) {
    ...
    const tiposInseguros = ocorrencias.filter((o) => o.tipo &&
        [
            'tipo-inseguro-any',
    ...
    for (const [tipo, ocorrenciasTipo] of Object.entries(porTipo)) {
        ...
        log.info(MENSAGENS_CLI_FIX_TYPES.linhaResumoTipo(formatarTipoInseguro(tipo, count)));
    }
    console.log();
    if (isDryRun) {
        ...
        for (const o of exemplos) {
            ...
            log.info(MENSAGENS_CLI_FIX_TYPES.exemploLinha(prefixo, o.relPath, String(o.linha || '?')));
            if (o.mensagem) {
        ...
        if (tiposInseguros.length > 10) {
            log.info(MENSAGENS_CLI_FIX_TYPES.maisOcorrencias(tiposInseguros.length - 10));
        }
        console.log();
    ...
    const { fixAnyToProperType } = await import('../../analistas/corrections/quick-fixes/fix-any-to-proper-type.js');
    const { fixUnknownToSpecificType } = await import('../../analistas/corrections/quick-fixes/fix-unknown-to-specific-type.js');
    ...
    for (const [arquivo, ocorrenciasArquivo] of Object.entries(porArquivo)) {
        ...
        for (const ocorrencia of ocorrenciasArquivo) {
            ...
            if (tipo === 'tipo-inseguro-any') {
                ...
                if (verbose) {
                    log.aviso(MENSAGENS_CLI_FIX_TYPES.verboseAnyDetectado(arquivo, String(ocorrencia.linha || '?')));
                }
                _erros++;
                continue;
            }
            if (tipo === 'tipo-inseguro-any-assertion') {
                ...
                if (verbose) {
                    log.erro(MENSAGENS_CLI_FIX_TYPES.verboseAsAnyCritico(arquivo, String(ocorrencia.linha || '?')));
                }
                _erros++;
                continue;
            }
            if (tipo === 'tipo-inseguro-any-cast') {
                ...
                stats.totalConfianca += 100;
                if (verbose) {
                    log.erro(MENSAGENS_CLI_FIX_TYPES.verboseAngleAnyCritico(arquivo, String(ocorrencia.linha || '?')));
                }
                _erros++;
                continue;
            }
            if (tipo === 'tipo-inseguro-unknown') {
                ...
                try {
                    ...
                    if (verbose) {
                        ...
                        log.info(MENSAGENS_CLI_FIX_TYPES.verboseUnknownCategoria(prefixo, arquivo, String(ocorrencia.linha || '?'), categorizacao.categoria, categorizacao.confianca));
                        log.info(MENSAGENS_CLI_FIX_TYPES.verboseMotivo(categorizacao.motivo));
                        if (categorizacao.sugestao) {
                            log.info(MENSAGENS_CLI_FIX_TYPES.verboseSugestao(categorizacao.sugestao));
                        }
                        if (categorizacao.variantes && categorizacao.confianca < 80) {
                            ...
                            categorizacao.variantes.forEach((variante, idx) => {
                                log.info(MENSAGENS_CLI_FIX_TYPES.verboseVarianteItem(idx + 1, variante));
                            });
                        }
                    }
                }
    ...
    if (verbose) {
        const fs = await import('node:fs/promises');
        const path = await import('node:path');
        const reportPath = path.join(PROJECT_ROOT, '.oraculo', 'fix-types-analise.json');
    ...
    if (altaPrioridade.length > 0) {
        log.fase?.(MENSAGENS_CLI_FIX_TYPES.altaConfiancaTitulo(altaPrioridade.length));
        log.info(MENSAGENS_CLI_FIX_TYPES.linhaEmBranco);
        for (const item of altaPrioridade.slice(0, 5)) {
            log.info(MENSAGENS_CLI_FIX_TYPES.altaConfiancaLinha(item.ocorrencia.relPath, String(item.ocorrencia.linha || '?'), item.confianca));
            log.info(MENSAGENS_CLI_FIX_TYPES.altaConfiancaDetalhe(item.sugestao || item.motivo));
        }
        if (altaPrioridade.length > 5) {
            log.info(MENSAGENS_CLI_FIX_TYPES.altaConfiancaMais(altaPrioridade.length - 5));
        }
        log.info(MENSAGENS_CLI_FIX_TYPES.linhaEmBranco);
    }
    ...
    if (casosIncertos.length > 0 && verbose) {
        ...
        for (const item of casosIncertos.slice(0, 3)) {
            log.info(MENSAGENS_CLI_FIX_TYPES.incertosLinha(item.ocorrencia.relPath, String(item.ocorrencia.linha || '?'), item.confianca));
    ...
    if (!isDryRun && altaPrioridade.length > 0) {
        ...
        if (resultado.sucesso > 0) {
            log.sucesso(MENSAGENS_CLI_FIX_TYPES.correcoesResumoSucesso(resultado.sucesso));
            for (const res of resultado.resultados) {
                if (res.sucesso && res.linhasModificadas > 0) {
                    log.info(MENSAGENS_CLI_FIX_TYPES.correcoesResumoLinhaOk(res.arquivo, res.linhasModificadas));
                }
                else if (!res.sucesso) {
                    log.erro(MENSAGENS_CLI_FIX_TYPES.correcoesResumoLinhaErro(res.arquivo, res.erro));
                }
            }
        }
        else {
    ...
    if (options.export || config.REPORT_EXPORT_ENABLED) {
        ...
        const casosParaExport = ocorrenciasCategorizadas.map((item) => ({
            ...
            contexto: 'contexto' in item.ocorrencia
                ? item.ocorrencia.contexto
                : undefined,
        }));
