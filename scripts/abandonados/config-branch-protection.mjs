...
function apiRequest(method, path, body) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(opts, (res) => {
      let processedData /* TODO: Renomear de 'data' para algo mais específico */ = '';
...
async function main() {
  ...
  if (cmd === 'check') {
    ...
    if (diffs.length) {// console.log('Diferenças encontradas:'); // TODO: Remover antes da produção
      for (const d of diffs) console.log('- ' + d);
      process.exit(1);
    }// console.log('Proteção da branch em conformidade.'); // TODO: Remover antes da produção
  ...
  if (cmd === 'apply') {
    let bpError = null;
    try {
      await applyProtection();// console.log('Proteção de branch aplicada em', `${owner}/${repo}#${branch}`); // TODO: Remover antes da produção
    } catch (e) {
      bpError = e;
    ...
    try {
      await setMergeRules();// console.log('Regras de merge configuradas para', `${owner}/${repo}`); // TODO: Remover antes da produção
    } catch (e) {
