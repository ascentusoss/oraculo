...
async function insertProvenienciaInPreview() {
  try {
    ...
    }// console.log('[preview] Aviso de proveniência inserido nos .md de preview-oraculo.'); // TODO: Remover antes da produção
  } catch (e) {
...
`;
  await fs.writeFile(path.join(OUT_DIR, 'PREVIEW.md'), previewNote, 'utf-8');// console.log('[preview] pronto. Veja a pasta preview-oraculo/.'); // TODO: Remover antes da produção
}
