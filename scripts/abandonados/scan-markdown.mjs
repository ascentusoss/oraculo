...
async function main() {
  ...
  for (const f of arquivos) console.log(path.relative(ROOT, f));// console.log(); // TODO: Remover antes da produção// console.log('==> Procurando padrões de risco de autoria/licença em .md'); // TODO: Remover antes da produção
  ...
  for (const f of arquivos) {
    ...
    if (whitelisted || marcadoOK) {
      ...
      continue;
    }// console.log(`RISCO: ${rel}`); // TODO: Remover antes da produção
    riscos++;
    if (/\/README\.md$/i.test(rel)) riscosReadme++;
  }
  if (!riscos) console.log('Nenhum padrão de risco encontrado.');// console.log(); // TODO: Remover antes da produção
  ...
  for (const f of arquivos) {
    ...
    if (!hasProvenienciaHeader(head)) {// console.log(`FALTA AVISO: ${rel}`); // TODO: Remover antes da produção
      missing++;
    }
  }
  ...
  if (missing) {
    ...
    process.exitCode = 1;
  } else {// console.log('\nOK: Varredura básica concluída.'); // TODO: Remover antes da produção
  }
